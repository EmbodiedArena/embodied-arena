...
class VILA(BaseAPIModel):
    ...
    def process_visuals(self, visual):
        if isinstance(visual, str) and visual.lower().endswith((".mp4", ".avi", ".mov")):
            try:
                ...
                eval_logger.warning(f"视频加载失败 {visual}: {e}")
                return None
        elif isinstance(visual, str) and visual.lower().endswith((".jpg", ".jpeg", ".png", ".bmp", ".tiff")):
            try:
                ...
                eval_logger.warning(f"图像加载失败 {visual}: {e}")
                return None
            ...
            try:
                ...
                eval_logger.warning(f"NumPy数组转换失败: {e}")
                return None
        else:
            eval_logger.warning(f"不支持的视觉类型: {type(visual)}")
            return None
    ...
    def respond(self, context, visuals, **gen_kwargs):
        ...
        if not processed_visuals:
            ...
            first_visual = processed_visuals[0]
            if first_visual["type"] == "image":
                task_type = "image"
                images = [v["pil_image"] for v in processed_visuals if v["type"] == "image"]
                question = f"{self.image_query}\n{context}"
                prompt = images + [question]
            else:  # video
                task_type = "video"
                video_path = first_visual["video_path"]
                from llava.media import Video
                video_obj = Video(video_path)
                self.model.config.fps = self.max_num_frames
                question = f"{self.video_query}\n{context}"
                prompt = [video_obj, question]
        ...
        try:
            ...
            # 创建 generation_config 对象
            generation_config = copy.deepcopy(self.model.default_generation_config)
            generation_config.max_new_tokens = effective_gen_kwargs["max_new_tokens"]
            generation_config.temperature = effective_gen_kwargs["temperature"]
            generation_config.top_p = effective_gen_kwargs["top_p"]
            generation_config.do_sample = effective_gen_kwargs["do_sample"]
            generation_config.num_beams = effective_gen_kwargs["num_beams"]
            generation_config.use_cache = effective_gen_kwargs["use_cache"]
