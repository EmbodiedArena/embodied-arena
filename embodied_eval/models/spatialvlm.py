...
class SpatialVLM(BaseAPIModel):
    ...
    def respond(self, context: str, visuals: List, **gen_kwargs) -> str:
        ...
        visual_list = visuals[0] if isinstance(visuals, tuple) else visuals

        image_tensor = None
        placeholder_count = 1

        if visual_list and isinstance(visual_list[0], Image.Image):
            image_tensor = process_images(visual_list, self._image_processor, self._config)
            if isinstance(image_tensor, list):
                image_tensor = [_image.to(dtype=torch.float16, device=self.device) for _image in image_tensor]
            else:
                image_tensor = image_tensor.to(dtype=torch.float16, device=self.device)

        elif visual_list and isinstance(visual_list[0], str):
            image_tensor = []
            try:
                frames = self.load_video(visual_list, self.max_num_frames)
