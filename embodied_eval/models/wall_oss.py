...
class Wall_OSS(BaseAPIModel):
    ...
    def __init__(
        ...
        # Follow vqa_inference.py: load model, then move to device with dtype in one call
    ...
    def process_visuals(self, visual):
        ...
        if isinstance(visual, str) and visual.lower().endswith((".mp4", ".avi", ".mov")):
            ...
            try:
                ...
                processed_visual = {"type": "image", "image": f"data:image/jpeg;base64,{b64}"}
            except Exception as e:
                eval_logger.warning(f"process_visuals: failed to load image {visual}: {e}")
    ...
    def respond(self, context, visuals, **gen_kwargs):
        ...
        # Move inputs to device (as per vqa_inference.py)
        ...
        if gen_kwargs.get("do_sample") and "temperature" not in gen_kwargs:
        ...
        # Use torch.no_grad() as per fake_inference.py (which works correctly)
        try:
            with torch.no_grad():
            ...
            eval_logger.error(f"Error during Wall-OSS generation: {e}")
        ...
        # Clear cache after generation to free up memory
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
