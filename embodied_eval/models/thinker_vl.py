...
@register_model("thinker_vl")
class Thinker_VL(BaseAPIModel):
    """
    Thinker-VL Model
    Wrapper around Qwen3VLForConditionalGeneration for UBTECH-Robotics/Thinker-4B.
    """

    def __init__(
        self,
        model_name_or_path: str = "UBTECH-Robotics/Thinker-4B",
        ...
        **kwargs,
        ...
        eval_logger.info(f"Loading Thinker-VL model from {model_name_or_path}")
        if use_flash_attention_2:
            try:
                ...
                eval_logger.warning(
                    f"Failed to load Thinker-VL with flash_attention_2: {e}, falling back to default attention implementation"
                )
                self._model = Qwen3VLForConditionalGeneration.from_pretrained(
                    model_name_or_path,
                    torch_dtype=torch.bfloat16,
        ...
        # Prefer model config max length when available
        self._max_length = (
            max_length
            if getattr(self._config, "max_length", None) is None
            else getattr(self._config, "max_length")
        )
        ...
        # Distributed evaluation wrapping
        if accelerator.num_processes > 1:
    ...
    def process_visuals(self, visual):
        """Convert different visual inputs into the format required by Qwen3/Thinker processor."""
    ...
    def build_messages(self, context: str, visuals: List[dict]) -> List[dict]:
        """Build messages in Qwen3/Thinker multi-modal chat format."""
        message = (
        ...
        if not self.interleave_visuals:
            ...
            # Interleave according to <image x> placeholders in context
    ...
    def respond(self, context, visuals, **gen_kwargs):
        ...
        # Process vision info and make sure fps is an int
        try:
            ...
            eval_logger.error(f"Error in process_vision_info for Thinker-VL: {e}")
            import traceback
        ...
        try:
            ...
            eval_logger.error(f"Error in Thinker-VL processor call: {e}")
            import traceback
