...
from transformers import Qwen2_5_VLProcessor, AutoTokenizer, Qwen2_5_VLForConditionalGeneration
...
@register_model("mimo_embodied")
class MimoEmbodied(BaseAPIModel):
    """
    MiMo-Embodied Model (based on Qwen2.5-VL architecture)
    Supports thinking mode and no_think mode.
    """

    def __init__(
            self,
            model_name_or_path: str = "XiaomiMiMo/MiMo-Embodied-7B",
        ...
        # Load model
        eval_logger.info(f"Loading MiMo-Embodied model from {model_name_or_path}")
    ...
    def build_messages(self, context: str, visuals: List[dict]) -> List[dict]:
        ...
        # Handle thinking mode: append /no_think suffix if thinking_mode is disabled
        if not self.thinking_mode:
            if not context.endswith("/no_think"):
                context = context + " /no_think"
