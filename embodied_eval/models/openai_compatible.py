...
class OpenAICompatible(BaseAPIModel):
    ...
    def respond(self, context: str, visuals: Union[List[str], List[Image.Image], List[tuple]], **gen_kwargs) -> str:
        ...
        if visuals:
            has_index = (
                isinstance(visuals, (list, tuple)) and
                len(visuals) == 2 and
                all(isinstance(img, Image.Image) for img in visuals[0]) and
                all(isinstance(i, int) for i in visuals[1])
            )

            if has_index:
                imgs.extend(visuals[0])
                visual_indices.extend(visuals[1])
            else:
