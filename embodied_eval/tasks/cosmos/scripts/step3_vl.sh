#!/bin/bash
# Cosmos Reasoning Benchmark - Step3-VL-10B-Base 评估脚本
...
# 模型配置
MODEL_PATH="/your/path/to/embodied-eval-main/embodied_eval/data/Step3-VL-10B-Base"
MODEL_TYPE="step3_vl"
OUTPUT_BASE="./logs/cosmos/step3_vl"

# 任务列表
TASKS=("bridgev2" "robovqa" "agibot" "holoassist" "robofail")

echo "================================"
echo "Cosmos - Step3-VL-10B-Base 评估"
echo "================================"
echo "模型: $MODEL_PATH"
echo "GPU: $GPUS"
echo "================================"
...
for task in "${TASKS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$GPUS python -m embodied_eval \
        --model $MODEL_TYPE \
        --model_args model_name_or_path=$MODEL_PATH,max_num_frames=8 \
        --evaluator eqa \
        --tasks cosmos-$task \
        --batch_size 1 \
