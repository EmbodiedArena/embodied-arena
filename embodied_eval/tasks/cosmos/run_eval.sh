#!/usr/bin/env bash
# Cosmos: 依次运行下方 RUN_SCRIPTS 中列出的 scripts/ 内脚本（仅 basename）。
# 约定：本文件与 scripts/ 目录始终同级；本脚本不修改当前工作目录。
# 子脚本的标准输出/错误输出直接打到当前终端。
# 本目录下 run_single_model.sh、quick_test.sh 不在 scripts/ 内，未列入。
