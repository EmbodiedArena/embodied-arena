...
CUDA_VISIBLE_DEVICES=7 accelerate launch --num_processes=1 --main_process_port=$PORT -m embodied_eval \
    --model mimo_embodied \
    --model_args model_name_or_path=XiaomiMiMo/MiMo-Embodied-7B,max_num_frames=1,use_flash_attention_2=True,thinking_mode=False \
    --evaluator eqa \
    --tasks unieqa \
    --batch_size 2 \
    --output_path ./logs/unieqa/mimo_embodied
